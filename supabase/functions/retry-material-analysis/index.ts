...
import { createRetryMaterialAnalysisHandler } from "../_shared/material_analysis/handlers.ts";

serve((request) => {
  const jwt = bearer(request);
  try {
    return createRetryMaterialAnalysisHandler(createAnalysisDependencies(jwt))(
      request,
    );
  } catch (_) {
    return unavailable();
